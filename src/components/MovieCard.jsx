...
export default function MovieCard({ item, onClick, appLang = 'ar' }) {
  ...
  return (
    <div 
      ...
      className="bg-slate-900/90 border border-slate-800/80 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-red-600/25 hover:-translate-y-2 transition-all duration-300 flex flex-col group cursor-pointer"
    >
      ...
      <div className="relative aspect-[2/3] overflow-hidden bg-slate-800">
        ...
        {/* التقييم */}
        <div className="absolute top-2 right-2 bg-slate-950/80 px-2 py-1 rounded-lg text-xs font-black text-amber-400 flex items-center gap-1 backdrop-blur-md border border-slate-700/50">
          ⭐ {item.vote_average ? item.vote_average.toFixed(1) : 'NEW'}
        </div>
        
        {/* شارة نوع العمل (فيلم أو مسلسل) */}
        <div className={`absolute top-2 left-2 ${badgeColor} px-2 py-1 rounded-lg text-xs font-bold backdrop-blur-md shadow-md`}>
          {mediaTypeText}
        </div>
      </div>
